...
class SlideManager {
    ...
    activateSlide(index) {
        ...
        // Mobile navbar control - ZMIENIONE: zawsze pokazuj navbar na mobile
        if (this.isMobile) {
    ...
    controlMobileNavbar() {
        ...
        if (navbar) {
            // ZMIENIONE: Zawsze pokazuj navbar na mobile, niezależnie od aktualnego slajdu
            navbar.style.opacity = '1';
            navbar.style.pointerEvents = 'auto';
            
            // Dodaj klasę dla pierwszego slajdu jeśli potrzebna do stylowania
            if (this.current === 0) {
                navbar.classList.add('on-first-slide');
            } else {
                navbar.classList.remove('on-first-slide');
            }
        }
    }
